...
class Agent:
    ...
    def reward(self, tree=None, reward=True):
        ...
        try:
            ...
            if length_of_expr <= 5 or length_of_expr > self.max_token:
                return 0 if reward else 1e999
            symbols = tree.get_exp()
            if self.exp_last:
                symbols = f"{self.exp_last}({symbols})"
            ans_tol, _ = cal_expression(symbols, self.config_s)
    ...
    def unavailable(self) -> List[int]:
        ...
        if self.tree.add_not_allowed > 0:
            exps.append("Add")
        if self.tree.head_token == "Denom":
            exps.extend(["Denom"])
